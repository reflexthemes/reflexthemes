...
class REFLEX_Theme {
	...
	private function general_files_include() {
		$files = array(
			...
			// Redux Subsection field linked
			'admin/settings/fields/subsection/class-redux-subsection',
	...
	private function settings_files_include() {
		$files = array(
			'page-title',
		);

		foreach ( $files as $file ) {
			$path = get_template_directory() . '/inc/settings/' . $file . '.php';
